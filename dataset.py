...
class dataset_unpair(data.Dataset):
    def __init__(self, opts):
        ...
        self.mask_size = 90
        # A
        dir_A = os.path.join(self.dataroot, 'label', opts.input_a + '_' + opts.phase)    ## data/INIT/label/sunny_train
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_size == self.A_size:
            ...
            cls_ = np.loadtxt(self.list_A[index], dtype=int, delimiter=' ', skiprows=boxn, usecols=(0))
            box_ = np.loadtxt(self.list_A[index], dtype=float, delimiter=' ', skiprows=boxn, usecols=(1, 2, 3, 4))
            if box_.size == 4:
                cls_ = np.expand_dims(cls_, axis=0)
                box_ = np.expand_dims(box_, axis=0)
            mask_ = self.make_mask(cls_, box_)
            data_A, mask_A = self.load_img(self.dataroot+name_A[0], self.input_dim_A, mask_)
            index1 = random.randint(0, self.B_size - 1)
            name_B = np.loadtxt(self.list_B[index1], dtype=str, delimiter=' ', skiprows=0, usecols=(0))
            cls_ = np.loadtxt(self.list_B[index1], dtype=int, delimiter=' ', skiprows=boxn, usecols=(0))
            box_ = np.loadtxt(self.list_B[index1], dtype=float, delimiter=' ', skiprows=boxn, usecols=(1, 2, 3, 4))
            if box_.size == 4:
                cls_ = np.expand_dims(cls_, axis=0)
                box_ = np.expand_dims(box_, axis=0)
            mask_ = self.make_mask(cls_, box_)
            data_B, mask_B = self.load_img(self.dataroot+name_B[0], self.input_dim_B, mask_)
        else:
            index1 = random.randint(0, self.A_size - 1)
            name_A = np.loadtxt(self.list_A[index1], dtype=str, delimiter=' ', skiprows=0, usecols=(0))
            cls_ = np.loadtxt(self.list_A[index1], dtype=int, delimiter=' ', skiprows=boxn, usecols=(0))
            box_ = np.loadtxt(self.list_A[index1], dtype=float, delimiter=' ', skiprows=boxn, usecols=(1, 2, 3, 4))
            if box_.size == 4:
                cls_ = np.expand_dims(cls_, axis=0)
                box_ = np.expand_dims(box_, axis=0)
            mask_ = self.make_mask(cls_, box_)
            ...
            cls_ = np.loadtxt(self.list_B[index], dtype=int, delimiter=' ', skiprows=boxn, usecols=(0))
            box_ = np.loadtxt(self.list_B[index], dtype=float, delimiter=' ', skiprows=boxn, usecols=(1, 2, 3, 4))
            if box_.size == 4:
                cls_ = np.expand_dims(cls_, axis=0)
                box_ = np.expand_dims(box_, axis=0)
            mask_ = self.make_mask(cls_, box_)
    ...
    def load_img(self, img_name, input_dim, mask):
        ...
        img = img.resize((self.size_w,self.size_h))   ### 추가
    ...
    def make_mask(self, cls, box):
        ...
        box_ = np.round(box * self.size_w/ 1920)
...
class testset(data.Dataset):
    ...
    def load_img(self, img_name, input_dim, mask):
        ...
        #img, mask = self.randomcrop(img, mask)        ### 뺴도 되나 이거
        img = self.transforms_i(img)
        mask_ = self.transforms_m(mask[0])
        for i in range(1, len(mask)):
            mask_ = torch.cat((mask_, self.transforms_m(mask[i])), 0)
