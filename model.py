...
class MGUIT(nn.Module):
    ...
    def generation(self, image_a, image_b, masks_a, masks_b, memory):  ###############################################################################################
        cont_a, cont_b = self.enc_c.forward(image_a, image_b)   # content
        sty_a, sty_b = self.memory.forward_second(cont_a, cont_b, masks_a, masks_b, memory) # attribute
        fake_A = self.gen.forward_a(cont_a, sty_a)
        fake_B = self.gen.forward_b(cont_a, sty_b)
        fake_C = self.gen.forward_a(cont_b, sty_b)
        fake_D = self.gen.forward_b(cont_b, sty_a)
        images_1 = self.normalize_image(fake_A).detach()
        images_2 = self.normalize_image(fake_B).detach()
        images_3 = self.normalize_image(fake_C).detach()
        images_4 = self.normalize_image(fake_D).detach()
        row1 = torch.cat((image_a[0:1, ::], images_1[0:1, ::], images_2[0:1, ::]),3)
        row2 = torch.cat((image_b[0:1, ::], images_3[0:1, ::], images_4[0:1, ::]),3)
